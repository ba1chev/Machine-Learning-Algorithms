...
std::vector<float> logistic_regression_params = { 0, 0, 0, 0 };
...
float logistic_regression(const std::vector<float>& input) {
    if (input.size() + 1 != logistic_regression_params.size()) {
    ...
    for (size_t i = 0; i < input.size(); i++) {
        result += input[i] * logistic_regression_params[i];
    }

    result += logistic_regression_params[input.size()];
...
float loss_function(const std::vector<float>& params) {
    logistic_regression_params = params;
    return binary_cross_entropy(X, Y, logistic_regression);
}

int main() {
    std::vector<float> start = { 0, 0, 0, 0 };
    auto result = gradient_descent(loss_function, start, 5000, 0.01);
    logistic_regression_params = result;
