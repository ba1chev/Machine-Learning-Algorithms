...
template <class T>
class BFGSOptimizer: public Optimizator<T> {
private:
...
template <class T>
Matrix<T> BFGSOptimizer<T>::compute_gradient(const LearningModel<T>* model, const Matrix<T>& weights,
    const Matrix<T>& X, const Matrix<T>& X_transposed, const Matrix<T>& y) const {
    switch (model->get_type()) {
        ...
        default: {
            throw std::runtime_error("BFGS does not support this model type");
        }
    }
}

template <class T>
T BFGSOptimizer<T>::compute_loss(const LearningModel<T>* model, const Matrix<T>& weights,
    const Matrix<T>& X, const Matrix<T>& y) const {
    Matrix<T> scores = X * weights;
...
template <class T>
T BFGSOptimizer<T>::dot(const Matrix<T>& column_a, const Matrix<T>& column_b) const {
    T sum = T(0);
...
template <class T>
void BFGSOptimizer<T>::optimize(LearningModel<T>* learning_model) const {
    if (!learning_model) {
    ...
    if (!supervised_model) {
        throw std::runtime_error("BFGS requires a supervised learning model");
    }
    ...
    Matrix<T> inverse_hessian = Matrix<T>::identity(count_of_features);
    ...
    for (size_t iteration = 0; iteration < this->iterations; iteration++) {
        ...
        Matrix<T> direction = (inverse_hessian * gradient) * T(-1);

        T directional_derivative = this->dot(gradient, direction);
        if (directional_derivative >= T(0)) {
            // Curvature lost (numerical drift): reset to steepest descent.
            inverse_hessian = Matrix<T>::identity(count_of_features);
        ...
        if (curvature > this->tolerance) {
            T rho = T(1) / curvature;
            Matrix<T> identity = Matrix<T>::identity(count_of_features);
            Matrix<T> left = identity - (weight_delta * gradient_delta.transpose()) * rho;
            Matrix<T> right = identity - (gradient_delta * weight_delta.transpose()) * rho;
            inverse_hessian = left * inverse_hessian * right + (weight_delta * weight_delta.transpose()) * rho;
        }
...
template <class T>
std::unique_ptr<Optimizator<T>> BFGSOptimizer<T>::clone() const {
    return std::make_unique<BFGSOptimizer<T>>(*this);
}

template <class T>
OptimizatorType BFGSOptimizer<T>::get_type() const {
    return OptimizatorType::BFGS;
}
