...
def generate_random_x_matrix(left_bound: float, right_bound: float, count_of_points: int, count_of_features: int) -> List[List[float]]:
    x_matrix = [[np.random.uniform(left_bound, right_bound) for _ in range(count_of_features)] for _ in range(count_of_points)]
    return x_matrix
...
def get_trained_n_params_model(X_matrix: List[List[float]], y_coordinates: List[float], model_training_lifecycle: int) -> tuple:
    n_features = len(X_matrix[0])
    parameters = [np.random.uniform(0, 1) for _ in range(n_features)]
    bias = np.random.uniform(0, 1)
    
    learning_rate = 0.01
    
    for _ in range(model_training_lifecycle):
        y_pred = [sum(parameters[j] * X_matrix[i][j] for j in range(n_features)) + bias for i in range(len(X_matrix))]
        gradients = [0 for _ in range(n_features)]
        bias_gradient = 0
        
        for i in range(len(X_matrix)):
            error = y_pred[i] - y_coordinates[i]
            
            for j in range(n_features):
                gradients[j] += 2 * error * X_matrix[i][j]
            
            bias_gradient += 2 * error
        
        for j in range(n_features):
            gradients[j] /= len(X_matrix)
        bias_gradient /= len(X_matrix)
        
        for j in range(n_features):
            parameters[j] -= gradients[j] * learning_rate
...
if __name__ == "__main__":
    ...
    print("\n=== Testing n-params model ===")
    x_matrix = generate_random_x_matrix(0, 10, 100, 3)
    
    real_function_n = lambda x: 2*x[0] + 3*x[1] + 1*x[2] + 5
    y_actual = [real_function_n(x_matrix[i]) for i in range(len(x_matrix))]
    y_noise_n = [y_actual[i] + np.random.normal(0, 2) for i in range(len(y_actual))]
    
    parameters, bias = get_trained_n_params_model(x_matrix, y_noise_n, 100)
    print(f"Trained parameters: {[f'{p:.2f}' for p in parameters]}")
    print(f"Trained bias: {bias:.2f}")
    print(f"True parameters: [2.00, 3.00, 1.00], bias: 5.00")
